import React from "react";
import { Package } from "lucide-react";

export interface InventoryCardProps {
  sku: string;
  onChangeSku: (val: string) => void;
  stockQty: string;
  onChangeStockQty: (val: string) => void;
  lowStockThreshold: string;
  onChangeLowStockThreshold: (val: string) => void;
  trackInventory: boolean;
  onToggleTrackInventory: (val: boolean) => void;
}

const FOCUS_RING =
  "focus:outline-none focus-visible:ring-2 focus-visible:ring-[#171717]/20 focus-visible:ring-offset-1";

export function InventoryCard({
  sku,
  onChangeSku,
  stockQty,
  onChangeStockQty,
  lowStockThreshold,
  onChangeLowStockThreshold,
  trackInventory,
  onToggleTrackInventory,
}: InventoryCardProps) {
  return (
    <div id="inventory" className="space-y-4 rounded-xl border border-[#E5E5E5] bg-white p-5 shadow-xs">
      <div className="flex items-center justify-between border-b border-[#E5E5E5] pb-3">
        <div>
          <h2 className="flex items-center gap-2 text-sm font-semibold text-[#171717]">
            <Package className="h-4 w-4 text-[#737373]" />
            <span>Inventory</span>
          </h2>
          <p className="mt-0.5 text-xs text-[#737373]">
            Track stock levels, SKUs, and low stock warnings.
          </p>
        </div>

        <label className="flex items-center gap-2 cursor-pointer text-xs font-medium text-[#171717]">
          <input
            type="checkbox"
            name="track_inventory"
            value="true"
            checked={trackInventory}
            onChange={(e) => onToggleTrackInventory(e.target.checked)}
            className="h-4 w-4 rounded border-[#E5E5E5] text-[#171717]"
          />
          <span>Track Stock</span>
        </label>
      </div>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div>
          <label className="block text-xs font-medium text-[#171717]">
            SKU Code
          </label>
          <input
            type="text"
            name="sku"
            value={sku}
            onChange={(e) => onChangeSku(e.target.value)}
            placeholder="e.g. TOY-WOOD-001"
            className={`mt-1.5 h-9 w-full rounded-lg border border-[#E5E5E5] bg-white px-3 font-mono text-xs text-[#171717] placeholder-[#A3A3A3] ${FOCUS_RING}`}
          />
        </div>

        <div>
          <label className="block text-xs font-medium text-[#171717]">
            Available Stock Quantity
          </label>
          <input
            type="number"
            name="stock_qty"
            min="0"
            disabled={!trackInventory}
            value={stockQty}
            onChange={(e) => onChangeStockQty(e.target.value)}
            placeholder="0"
            className={`mt-1.5 h-9 w-full rounded-lg border border-[#E5E5E5] bg-white px-3 font-mono text-xs text-[#171717] placeholder-[#A3A3A3] disabled:bg-[#FAFAF9] disabled:text-[#A3A3A3] ${FOCUS_RING}`}
          />
        </div>

        <div>
          <label className="block text-xs font-medium text-[#171717]">
            Low Stock Threshold
          </label>
          <input
            type="number"
            name="low_stock_threshold"
            min="0"
            disabled={!trackInventory}
            value={lowStockThreshold}
            onChange={(e) => onChangeLowStockThreshold(e.target.value)}
            placeholder="5"
            className={`mt-1.5 h-9 w-full rounded-lg border border-[#E5E5E5] bg-white px-3 font-mono text-xs text-[#171717] placeholder-[#A3A3A3] disabled:bg-[#FAFAF9] disabled:text-[#A3A3A3] ${FOCUS_RING}`}
          />
        </div>
      </div>
    </div>
  );
}
