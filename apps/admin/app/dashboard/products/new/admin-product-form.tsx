...
import { CatalogOwnershipCard, type SellerOption } from "./components/catalog-ownership-card";
...
export function AdminProductForm({ sellers, adminUserId }: AdminProductFormProps) {
  ...
  const [category, setCategory] = useState<string>(TOY_CATEGORIES[0] || "Wooden Toys");
  ...
  // 3. Media State (Unified 8-image gallery)
  ...
  // 5. Tax & Compliance State
  const [gstRate, setGstRate] = useState("18");
  const [hsnCode, setHsnCode] = useState("");
  const [countryOfOrigin, setCountryOfOrigin] = useState("India");

  // 6. Merchandising State
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 pb-16">
      {/* PAGE HEADER & ACTIONS */}
      <div className="flex flex-col gap-4 rounded-xl border border-[#E5E5E5] bg-white p-4 shadow-xs sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2 text-xs text-[#737373] mb-1">
            <Link href="/admin/dashboard/products" className="hover:text-[#171717] hover:underline flex items-center gap-1">
              <ArrowLeft className="h-3.5 w-3.5" />
              <span>Products</span>
            </Link>
            <span>/</span>
            <span className="font-semibold text-[#171717]">Create Listing</span>
          </div>
          <h1 className="text-xl font-semibold tracking-tight text-[#171717]">
            Create Product
          </h1>
          <p className="text-xs text-[#737373] mt-0.5">
            Build and publish a complete product listing for your catalog.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            disabled={isPending}
            className={`h-9 items-center gap-1.5 rounded-lg border-[#E5E5E5] bg-white px-3.5 text-xs font-medium text-[#171717] hover:bg-[#F5F5F4] ${PRESSABLE}`}
          >
      ...
      <form id="enterprise-product-form" onSubmit={(e) => { e.preventDefault(); executeSubmit(); }} noValidate className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-[#E5E5E5]">
          <Button
            ...
            disabled={isPending}
            className={`h-10 items-center gap-1.5 rounded-lg border-[#E5E5E5] bg-white px-4 text-xs font-medium text-[#171717] hover:bg-[#F5F5F4] ${PRESSABLE}`}
          >
      ...
      {showPreviewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <div className="relative w-full max-w-lg rounded-2xl border border-[#E5E5E5] bg-white p-6 shadow-xl">
            <button
              type="button"
