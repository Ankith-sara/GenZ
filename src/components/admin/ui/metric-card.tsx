...
export function MetricCard({
  ...
  // Normalize sparkline SVG path to strictly fit inside viewBox [0 0 64 20]
  const sparklinePath = useMemo(() => {
    ...
    return sparklineData
      .map((val, idx) => {
        ...
        // Map Y to padding range 4px (top) to 16px (bottom)
    ...
    >
      <div>
        <div className="flex items-center justify-between gap-2">
          <span className="font-graphik text-xs font-semibold tracking-wider text-[#73736E] uppercase">
            {title}
          </span>
          {icon && (
        ...
        <div className="mt-3 flex items-baseline justify-between gap-2">
          <span className="font-graphik text-2xl font-bold tracking-tight text-[#1A1A18] sm:text-3xl">
            {value}
          </span>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-[#F0F0EC] pt-3">
        {description ? (
          <span className="font-graphik max-w-[140px] truncate text-[11px] text-[#8C8C85]">
            {description}
          </span>
        ) : (
          <span className="font-graphik text-[11px] text-[#8C8C85]">
            vs previous 7 days
          </span>
        )}

        {/* Scaled SVG Sparkline */}
        {sparklinePath && (
