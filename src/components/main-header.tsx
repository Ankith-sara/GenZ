  ...
  return (
    <>
      ...
      <div className="bg-[#07170f] text-gold-yellow py-2 px-4 text-center text-xs tracking-wider font-sans border-b border-white/5 flex items-center justify-center gap-1.5 hover:text-white transition-colors">
        ...
        <Link href="/discover?innovations=true" className="font-bold underline flex items-center gap-0.5">
          ...
          <div className="flex h-24 items-center justify-between">
            ...
            <Link href="/" className="flex items-center gap-3 group">
              ...
              <div className="flex flex-col text-left">
                <span className="text-xl font-bold tracking-tight text-white leading-none">GenZ</span>
            ...
            <nav className="hidden md:flex items-center gap-8">
              ...
              <div className="relative" ref={resourcesRef}>
                ...
                {showResources && (
                  <div className="absolute right-0 mt-3 w-80 rounded-lg bg-white text-black shadow-2xl ring-1 ring-black/5 p-4 z-50 grid grid-cols-1 gap-3 border border-gray-100 animate-fade-in">
                    <Link
                      href="/about"
                      className="flex items-start gap-3 p-2 rounded-md hover:bg-light-gray-bg hover:text-forest-green transition-colors"
                      onClick={() => setShowResources(false)}
                    >
                      <div className="bg-forest-green/5 text-forest-green p-2 rounded-md shrink-0">
              ...
              {isLoggedIn ? (
                ...
                <div className="relative" ref={userMenuRef}>
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/20 hover:border-gold-yellow bg-white/5 hover:bg-white/10 text-white transition-all focus:outline-none"
                  >
                    <div className="h-6 w-6 bg-gold-yellow text-forest-green rounded-full flex items-center justify-center text-xs font-bold font-serif shadow-inner">
                  ...
                  {showUserMenu && (
                    <div className="absolute right-0 mt-3 w-56 rounded-lg bg-white text-black shadow-2xl ring-1 ring-black/5 py-1.5 z-50 border border-gray-100 animate-fade-in text-left">
                      <div className="px-4 py-2 border-b border-gray-100">
                        <p className="text-xs text-neutral-400">Signed in as</p>
                        <p className="text-xs font-bold truncate text-forest-green">{userName || role || "User"}</p>
                      </div>
                      <Link
                ...
                <>
                  <Button asChild variant="outline" className="border-white/30 text-white hover:bg-white/10 hover:text-gold-yellow rounded-[4px] h-11 px-5 text-sm font-semibold tracking-wider bg-transparent transition-all">
            ...
            <div>
              ...
              <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-6">
                <Link href="/" className="flex items-center gap-2" onClick={() => setIsOpen(false)}>
                  <div className="w-8 h-8 bg-gold-yellow flex items-center justify-center text-forest-green font-black text-xl rounded-[4px]">
                    Z
                  </div>
                  <span className="text-lg font-bold text-white tracking-tight">GenZ</span>
                </Link>
                <button
                  onClick={() => setIsOpen(false)}
                  className="rounded-md p-1.5 text-white/70 hover:bg-white/10 focus:outline-none"
              ...
              <nav className="flex flex-col gap-2">
                <Link
                  href="/"
                  className={`flex items-center gap-3 text-base font-semibold px-4 py-3 rounded-md hover:bg-white/5 transition-colors ${
                    pathname === "/" ? "text-gold-yellow bg-white/5" : "text-white/90"
                  }`}
                  onClick={() => setIsOpen(false)}
                >
                ...
                <Link
                  href="/about"
                  className={`flex items-center gap-3 text-base font-semibold px-4 py-3 rounded-md hover:bg-white/5 transition-colors ${
                    pathname === "/about" ? "text-gold-yellow bg-white/5" : "text-white/90"
                  }`}
                  onClick={() => setIsOpen(false)}
                >
                ...
                <Link
                  href="/discover"
                  className={`flex items-center gap-3 text-base font-semibold px-4 py-3 rounded-md hover:bg-white/5 transition-colors ${
                    pathname === "/discover" && !pathname.includes("innovations") ? "text-gold-yellow bg-white/5" : "text-white/90"
                  }`}
                  onClick={() => setIsOpen(false)}
                >
                ...
                <Link
                  ...
                  className={`flex items-center gap-3 text-base font-semibold px-4 py-3 rounded-md hover:bg-white/5 transition-colors ${
                    pathname.includes("innovations") ? "text-gold-yellow bg-white/5" : "text-white/90"
                  }`}
                  onClick={() => setIsOpen(false)}
                >
              ...
              {isLoggedIn ? (
                <>
                  <div className="text-center mb-2 px-2">
                    ...
                    <p className="text-xs font-bold truncate text-gold-yellow">{userName || role || "User"}</p>
                  </div>
                  <Button asChild variant="outline" className="w-full border-white/30 text-white hover:bg-white/10 rounded-[4px] h-10 text-xs font-medium uppercase tracking-wider bg-transparent">
