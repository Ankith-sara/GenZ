...
import { 
  Building2, ArrowRight, ArrowLeft, Check, Upload, ShieldAlert, CheckCircle2 
} from "lucide-react";
...
export function ManufacturerSignupForm() {
  ...
  // Form state
  const [state, formAction, isPending] = useActionState<ManufacturerSignupState, FormData>(
    signupManufacturer,
    {}
  );
  ...
  const MANUFACTURER_STEPS = [
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              {!phoneVerified ? (
                ...
                <div className="flex items-center gap-1 text-green-600 bg-green-50 px-3 py-1.5 rounded-[4px] border border-green-200 text-sm font-semibold shrink-0">
                  <CheckCircle2 className="h-4 w-4" /> Verified
                </div>
              )}
            </div>
            {otpSent && !phoneVerified && (
              <div className="mt-2 flex gap-2 items-center bg-gray-50 p-3 rounded border">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex gap-4 mt-2">
                <label className="flex items-center gap-1.5 text-sm">
                  <input type="radio" name="export_available" value="Yes" defaultChecked /> Yes
                </label>
                <label className="flex items-center gap-1.5 text-sm">
    ...
    },
    {
      ...
      fields: (
        <div className="space-y-4">
          {["gst_certificate_file", "udyam_certificate_file", "factory_license_file"].map((field) => (
            <div key={field} className="border p-3 rounded bg-gray-50 flex flex-col gap-2">
              <Label className="capitalize font-semibold text-xs text-charcoal">
                {field.replace(/_/g, " ")} {field === "factory_license_file" ? "(Optional)" : "(Required)"}
              </Label>
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="text-xs text-smoke truncate max-w-xs">
                  {uploadedFiles[field] ? (
                    <span className="text-green-600 font-medium">✓ {uploadedFiles[field]}</span>
    ...
    },
    {
      ...
      fields: (
        <div className="space-y-4">
          <p className="text-xs text-smoke">Upload photos of your company setup to build trust (under 10MB each).</p>
          {["company_logo", "factory_exterior", "factory_interior", "machinery_photo", "production_line"].map((field) => (
            <div key={field} className="border p-3 rounded bg-gray-50 flex flex-col gap-2">
              <Label className="capitalize font-semibold text-xs text-charcoal">
      ...
      fields: (
        <div className="space-y-4">
          ...
          <div className="border p-3 rounded bg-gray-50 flex flex-col gap-2">
            <Label className="font-semibold text-xs text-charcoal">
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="text-xs text-smoke truncate max-w-xs">
                {uploadedFiles["walkthrough_video"] ? (
                  <span className="text-green-600 font-medium">✓ {uploadedFiles["walkthrough_video"]}</span>
            ...
            {uploadedFiles["walkthrough_video"] && (
              <input type="hidden" name="walkthrough_video" value={uploadedFiles["walkthrough_video"]} />
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {[
              ...
              <label key={idx} className="flex items-start gap-2.5 text-xs text-charcoal py-0.5 select-none cursor-pointer">
                <input type="checkbox" name={`video_inclusion_${idx}`} value="Yes" defaultChecked className="mt-0.5 rounded border-gray-300" />
                <span>{item}</span>
              </label>
            ))}
          </div>
  ...
  const STARTUP_FIELDS = (
    <div className="space-y-4 text-left">
      ...
      {/* Logo upload */}
      <div className="border p-3 rounded bg-gray-50 flex flex-col gap-2">
        <Label className="font-semibold text-xs text-charcoal">Brand Logo (Required)</Label>
        <div className="flex items-center justify-between gap-4">
          <input 
            type="file" 
            id="brand_logo" 
          ...
          <div className="text-xs text-smoke">
            {uploadedFiles["brand_logo"] ? (
              <span className="text-green-600 font-medium">✓ {uploadedFiles["brand_logo"]}</span>
  ...
  const ARTISAN_FIELDS = (
    <div className="space-y-4 text-left">
      ...
      {/* Workshop Photos */}
      <div className="border p-3 rounded bg-gray-50 flex flex-col gap-2">
        <Label className="font-semibold text-xs text-charcoal">Workshop Photos (Required)</Label>
        <div className="flex items-center justify-between gap-4">
          <input 
            type="file" 
            id="workshop_photos" 
          ...
          <div className="text-xs text-smoke">
            {uploadedFiles["workshop_photos"] ? (
              <span className="text-green-600 font-medium">✓ {uploadedFiles["workshop_photos"]}</span>
        ...
        {uploadedFiles["workshop_photos"] && (
          <input type="hidden" name="workshop_photos" value={uploadedFiles["workshop_photos"]} />
        )}
      </div>

      {/* Product Photos */}
      <div className="border p-3 rounded bg-gray-50 flex flex-col gap-2">
        <Label className="font-semibold text-xs text-charcoal">Product Photos (Required)</Label>
        <div className="flex items-center justify-between gap-4">
          <input 
            type="file" 
            id="artisan_products" 
          ...
          <div className="text-xs text-smoke">
            {uploadedFiles["artisan_products"] ? (
              <span className="text-green-600 font-medium">✓ {uploadedFiles["artisan_products"]}</span>
        ...
        {uploadedFiles["artisan_products"] && (
          <input type="hidden" name="artisan_products" value={uploadedFiles["artisan_products"]} />
        )}
      </div>

      {/* Crafting Process Video */}
      <div className="border p-3 rounded bg-gray-50 flex flex-col gap-2">
        <Label className="font-semibold text-xs text-charcoal">Short video showing the crafting/manufacturing process (Required)</Label>
        <div className="flex items-center justify-between gap-4">
          <input 
            type="file" 
            id="crafting_video" 
          ...
          <div className="text-xs text-smoke">
            {uploadedFiles["crafting_video"] ? (
              <span className="text-green-600 font-medium">✓ {uploadedFiles["crafting_video"]}</span>
        ...
        {uploadedFiles["crafting_video"] && (
          <input type="hidden" name="crafting_video" value={uploadedFiles["crafting_video"]} />
        )}
      </div>
    </div>
  );
  ...
  if (businessType === "startup" || businessType === "artisan") {
    return (
      <form action={formAction} noValidate className="animate-fade-in">
        ...
        {state?.error && (
          <p role="alert" className="text-destructive text-sm font-semibold mt-4 text-left">
            {state.error}
          </p>
        )}
  ...
  return (
    <form action={formAction} noValidate className="animate-fade-in text-left">
      ...
      {state?.error && isLastStep && (
        <p role="alert" className="text-destructive text-sm font-semibold mt-4">
          {state.error}
        </p>
      )}
      ...
      <div className="mt-8 flex gap-3">
        {step > 1 && (
          <Button 
            ...
            variant="outline"
            onClick={handlePrevStep}
            className="flex-1 border-forest-green text-forest-green hover:bg-forest-green/5 h-11"
          >
            Previous
          </Button>
        )}
