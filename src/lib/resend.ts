/**
 * Resend Email Integration Utility
 */
...
export async function sendResendEmail(
  ...
  try {
    ...
    console.log(
      `[Resend] Email successfully dispatched to ${options.to} (ID: ${body?.id || "N/A"})`
    );
...
/**
 * Helper to format and dispatch approved seller credentials email via Resend.
 */
export async function sendSellerApprovalEmail(
  params: SellerApprovalEmailParams
): Promise<SendEmailResult> {
  const siteUrl =
    params.siteUrl || process.env.NEXT_PUBLIC_SITE_URL || "https://genzonline.in";
